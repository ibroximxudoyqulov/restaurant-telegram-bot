...
# ============================================================
#  ⚙️  СОЗЛАМАҲОИ АСОСӢ (CONFIG)
# ============================================================
TOKEN    = os.environ.get("BOT_TOKEN", "TOKEN_БАР_ИН ҶО")
...
# ============================================================
#  🛠️  ФУНКСИЯҲОИ ЁРИДИҲАНДА (ВАСИЛАҲОИ ВАҚТИ ДУШАНБЕ)
# ============================================================
def get_dushanbe_now() -> datetime:
    dushanbe_tz = timezone(timedelta(hours=5))
    return datetime.now(dushanbe_tz)

def is_open() -> bool:
    return OPEN_HOUR <= get_dushanbe_now().hour < CLOSE_HOUR
...
def admin_router(msg):
    ...
    if txt == "👑 Саҳифаи Админ":
        ...
        conn  = get_conn()
        c     = conn.cursor()
        today = get_dushanbe_now().strftime("%Y-%m-%d")
        ...
        bot.send_message(
            ...
            f"👤 Мизоҷон (Ҷамъи аъзоён): *{users}* нафар\n"
...
def back_to_main(msg):
    bot.send_message(msg.chat.id, "🏠 Menuи асосӣ:", reply_markup=main_kb())

# ============================================================
#  📱 КЛИЕНТ РОУТИНГ (CLIENT ROUTER)
# ============================================================
CLIENT_BTNS = {
    "📋 Менюи таомҳо",
    "🛒 Сабади харид",
...
def cb_cart(call):
    ...
    if action == "add":
        ...
        if row:
            ...
            conn.commit()
            bot.answer_callback_query(call.id, f"✅ {food_name} ба saбад илова шуд!")
        else:
            bot.answer_callback_query(call.id, "❌ Таом ёфт нашуд.")
    else:
        c.execute(
...
def after_phone(uid: int):
    if order_mode.get(uid) == "gift":
        m = bot.send_message(uid, "👤  Рақами телефони қабулкунандаи тӯҳфаро ворид кунед (масалан: +992XXXXXXXXX):")
        bot.register_next_step_handler(m, process_gift_receiver)
    else:
        ask_service(uid)

# ── Тӯҳфа: ёфтани қабулкунанда дар база ──────────────────
def process_gift_receiver(msg):
    ...
    receiver_id = None
    for u_id, u_phone in all_users:
        if u_phone:
            u_phone_clean = re.sub(r"[^\d]", "", u_phone)
            if u_phone_clean.endswith(last_9_digits):
                receiver_id = u_id
                break
    conn.close()
    ...
    if not receiver_id:
        bot.send_message(
            uid,
            "❌ Ин рақами телефон ҳанӯз дар боти мо рӯйхат нашудааст (Корбар бояд ақаллан як бор ботро оғоз карда бошад)!\n"
...
def finalize_order(sender_id, receiver_id, o_type, details, code=None):
    ...
    date_str = get_dushanbe_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    # Xatolik to'g'rilandi: Jadvaldagi barcha 8 ta ustun nomi va tartibi aniq ko'rsatildi
    c.execute(
        "INSERT INTO orders (code, user_id, receiver_id, type, details, total, status, date_time) "
        "VALUES (?, ?, ?, ?, ?, ?, '🆕 Нав', ?)",
...
# ============================================================
#  📅 БРОН КАРДАНИ СТОЛ (TABLE BOOKING)
# ============================================================
...
# ============================================================
#  📊 ҲИСОБОТИ РӮЗОНА — ҳар кеч соат 21:00 (БЕХАТАР ТЕКШИРУВ)
# ============================================================
def daily_report_loop():
    ...
    while True:
        ...
        # Agar soat 21:00 bo'lsa va hali hisobot berilmagan bo'lsa
        if now.hour == 21 and now.minute == 0 and not reported_today:
            try:
                conn  = get_conn()
                c     = conn.cursor()
        ...
        # Soat 21:01 da flagni tushiramiz, ertasi kuni yana ishlashi uchun
        if now.hour == 21 and now.minute == 1:
            reported_today = False
            
        time.sleep(30) # Har 30 soniyada tekshiradi (Render uchun eng xavfsiz rejim)

# ============================================================
#  🚫 ФИЛТРИ ПАЁМҲОИ БЕҲУДА (SPAM & UNKNOWN)
# ============================================================
...
# Tuzatildi: func=lambda filteri faqat tizimli tugmalardan tashqari va kutilmagan matnlarni ushlaydi.
# register_next_step_handler bilan to'qnashib ketmaydi.
@bot.message_handler(func=lambda m: m.text not in CLIENT_BTNS and m.text not in ADMIN_BTNS and m.text != "⬅️ Ба менюи асосӣ")
def handle_unknown(msg):
    uid = msg.chat.id

    # Admin taom qo'shayotgan bo'lsa, ushbu jarayonni buzmaslik
