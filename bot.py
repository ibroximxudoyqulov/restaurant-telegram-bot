...
# ============================================================
TOKEN    = os.environ.get("BOT_TOKEN", "7434316045:AAFi_HwXW-pUX9mZgN30iYwK69m_D_N0_Q8")
ADMIN_ID = int(os.environ.get("ADMIN_ID", "1753381665"))
...
def check_session(uid: int) -> bool:
    ...
    if row and (now - row[0] > SESSION_TIMEOUT):
        ...
        bot.send_message(
            uid,
            "⏱️ *Вақти сессияи шумо ба охир расид!*\n"
            "Сабади харидатон автоматӣ тоза карда шуд.\n"
            "Шумо метавонед аз нав фармоишро оғоз кунед.",
            parse_mode="Markdown"
        )
        return False
...
def client_router(msg):
    uid = msg.chat.id
    check_session(uid)  # Сессия фақат сабадро тоза мекунад, лекин тугмаҳоро блок намекунад!
...
def process_gift_receiver(msg):
    ...
    if not row:
        ...
        bot.send_message(
            ...
            f"Азбаски ин шахс ҳанӯз дар бот нест, баробари ворид шудан ва пахш кардани /start, бот аз у локатсия мепурсад.",
            reply_markup=main_kb()
        )
        return
...
def handle_location(msg):
    ...
    if uid in pending_orders:
        ...
        if not in_dushanbe(lat, lon):
            bot.send_message(uid, "❌ Шумо берун аз ҳудуди шаҳри Душанбе ҳастед! Бекор карда шуд.", reply_markup=main_kb())
            bot.send_message(info["sender_id"], "❌ Қабулкунанда берун аз Душанбе аст. Фармоиш бекор шуд.", reply_markup=main_kb())
            return
    ...
    if not in_dushanbe(lat, lon):
        ...
        conn.commit()
        conn.close()
        bot.send_message(uid, "❌ Шумо берун аз ҳудуди Душанбе ҳастед! Сабад тоза карда шуд.", reply_markup=main_kb())
        return
    ...
    if stype == "🍽️ Дар зали ресторан":
        ...
        if dist > 150:
            ...
            conn.commit()
            conn.close()
            bot.send_message(uid, f"❌ Шумо дар зали ресторан нестед! Масофа: {dist:.0f}м. Сабад тоза шуд.", reply_markup=main_kb())
            return
...
# ============================================================
#  📅 БРОН КАРДАНИ СТОЛ (МАКСИМУМ 1 МОҲ ЧЕКЛОВ)
# ============================================================
def start_booking(uid: int):
    ...
    booking_state[uid]["phone"] = row[0] if row else ""
...
def step_guests(msg):
    ...
    if guests > 50:
        m = bot.send_message(uid, "❌ Максимум 50 нафар ворид кардан мумкин аст:")
        bot.register_next_step_handler(m, step_guests)
        return
...
def step_day(msg):
    ...
    if not msg.text or not msg.text.strip().isdigit():
        ...
        bot.register_next_step_handler(m, step_day)
        return
    booking_state[uid]["day"] = int(msg.text.strip())
    m = bot.send_message(uid, "🗓️ Моҳро бо рақам ворид кунед (масалан: 6):")
...
def step_month(msg):
    ...
    # Агар моҳ аз моҳи жорийдан кичик бўлса, демак кейинги йил
    if month < now.month:
        year += 1

    try:
        target_date = datetime(year, month, day, now.hour, now.minute)
        # Қатъий 1 ойлик чеклов (30 кун)
        max_allowed_date = now + timedelta(days=30)
        
        if target_date < now:
            bot.send_message(uid, "❌ Шумо наметавонед ба вақти гузашта стол брон кунед!", reply_markup=main_kb())
            booking_state.pop(uid, None)
            return
            
        if target_date > max_allowed_date:
            bot.send_message(uid, "❌ Столро танҳо то 1 моҳ (30 рӯз) пешакӣ брон кардан мумкин аст! Аз ин муддат 1 соат ҳам гузаштани он иҷозат нест.", reply_markup=main_kb())
            booking_state.pop(uid, None)
            return

    except ValueError:
        bot.send_message(uid, "❌ Санаи нодуруст ворид карда шуд.", reply_markup=main_kb())
        booking_state.pop(uid, None)
        return

    booking_state[uid]["valid_date"] = target_date.strftime("%Y-%m-%d")
...
def step_time(msg):
    ...
    if not data:
        bot.send_message(uid, "❌ Хатогии сессия.", reply_markup=main_kb())
        return
...
def handle_spam(msg):
    ...
    bot.send_message(msg.chat.id, "⚠️ Лутфан танҳо аз тугмаҳои мавҷуда истифода баред!", reply_markup=mk)
