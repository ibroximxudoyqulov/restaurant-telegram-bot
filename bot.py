...
# ============================================================
#  ⚙️  ТАНЗИМОТИ АСОСӢ ВА WEB-SERVER БАРОИ RENDER
# ============================================================
TOKEN    = os.environ.get("BOT_TOKEN", "7485942548:AAFS_ExampleToken")  # Токени худро дар Render гузоред
...
# Flask сохтан барои устувории Render (Port 10000 стандартӣ)
app = Flask(__name__)

@app.route('/')
def home():
    return "Бот бо муваффақият 24/7 фаъол аст!", 200
...
# ============================================================
#  🗄️  БАЗАИ МАЪЛУМОТҲО (БАЗА ДАННЫХ)
# ============================================================
def get_conn():
...
def save_initial_phone(msg):
    ...
    if not msg.contact:
        mk = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
        mk.add(types.KeyboardButton("📱 Тасдиқи рақами telephone", request_contact=True))
...
def check_pending_gifts_for_user(uid: int, phone: str):
    ...
    if row:
        ...
        bot.send_message(
            ...
            "Лутфан локатсияи воқеии (живой) худро бифиристед то буюртмаро онем:",
            reply_markup=mk
        )
    else:
        conn.close()
...
# ============================================================
#  📱  РОУТИНГИ КЛИЕНТ ВА КУШОДАНИ ПРОФИЛ
# ============================================================
CLIENT_BTNS = {
    "📋 Менюи таомҳо",
    "🛒 Сабади харид",
...
def process_gift_receiver(msg):
    ...
    if not row:
        ...
        bot.send_message(
            ...
            f"Азбаски ин шахс ҳанӯз дар бот нест, баробари ворид шудан ва пахш кардани /start, бот аз ӯ локатсияи зиндаро мепурсад.",
            reply_markup=main_kb()
        )
        return
    ...
    bot.send_message(uid, "⏳ Рақам тасдиқ шуд. Мунтазири локатсияи воқеии қабулкунанда...", reply_markup=main_kb())

    mk = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
    mk.add(types.KeyboardButton("📍 Фиристодани локатсия", request_location=True))
    bot.send_message(
        receiver_id,
        "🎁 Салом! Дӯстатон мехоҳед ба шумо тӯҳфа бифиристад.\n"
        "Лутфан локатсияи воқеии (живой) худро бифиристед:",
        reply_markup=mk
    )
...
def save_service(msg):
    ...
    mk = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
    mk.add(types.KeyboardButton("📍 Фиристодани локатсия", request_location=True))
    bot.send_message(uid, "📍 Локатсияи воқеии (Live Location) худро бифиристед:", reply_markup=mk)

# ============================================================
#  📍  ЛОКАТСИЯ (МАНТИҚИ НАВ БЕ ОЧИРИШИ САВАД ВА ТАЛАБИ ЖИВОЙ)
# ============================================================
...
def handle_location(msg):
    ...
    if uid in pending_orders:
        ...
        if not in_dushanbe(lat, lon):
            ...
            bot.send_message(
                uid,
                "❌ *Хатогии минтақа!*\n\n"
                "Шумо берун аз ҳудуди шаҳри Душанбе ҳастед! Ресторан танҳо дар Душанбе хизмат мерасонад.\n"
                "Лутфан, локатсияи дурустро ворид кунед 👇",
                reply_markup=mk
            )
            return

        info_pop = pending_orders.pop(uid)
        finalize_order(
            sender_id   = info_pop["sender_id"],
            receiver_id = uid,
            o_type      = "🎁 Тӯҳфа",
    ...
    # 1. Танҳо ҳудуди Душанбе (САБАД НЕСТ КАРДА НАМЕШАВАД)
    if not in_dushanbe(lat, lon):
        mk = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
        mk.add(types.KeyboardButton("📍 Фиристодани локатсия", request_location=True))
        bot.send_message(
            uid,
            "⚠️ *Огоҳӣ: Самарбахш нест!*\n\n"
            "Локатсияи фиристодаи шумо берун аз марзи Душанбе аст.\n"
            "Таомҳо дар сабад бехатаранд! Лутфан локатсияи воқеии дохили шаҳрро бифиристед 👇",
            parse_mode="Markdown",
            reply_markup=mk
        )
        return

    # 2. Дар зал — санҷиши масофа (САБАД НЕСТ КАРДА НАМЕШАВАД)
    if stype == "🍽️ Дар зали ресторан":
        ...
        if dist > 150:
            ...
            bot.send_message(
                uid,
                f"⚠️ *Шумо дар ресторан нестед!*\n\n"
                f"Масофа то ресторан: {dist:.0f} метр аст (Максимум иҷозат: 150м).\n"
                "Сабади шумо тоза нашуд. Вақте ба ресторан омадед, дубора локатсия бифиристед 👇",
                parse_mode="Markdown",
                reply_markup=mk
            )
            return
...
def start_booking(uid: int):
    ...
    booking_state[uid]["phone"] = row[0] if row else "Номаълум"
