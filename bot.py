...
#  ⚙️  ТАНЗИМОТИ АСОСӢ
# ============================================================
TOKEN    = os.environ.get("BOT_TOKEN", "7437894314:AAHjG...") # Tokeningizni shu yerga yozing yoki Environment Variables'ga qo'ying
ADMIN_ID = int(os.environ.get("ADMIN_ID", "0")) # ID-i admini botро ворид кунед
...
def init_db():
    ...
    # Базаро бо таомҳо ва ID-и расми Ош пур мекунем
    c.execute("SELECT COUNT(*) FROM menu")
    if c.fetchone()[0] == 0:
        # Шумо фиристодаги расми Ош (File ID) шуд: "AgACAgIAAxkBAAEY..."
        OSH_IMAGE_ID = "AgACAgIAAxkBAAEYu_pmZ2jS2HjW_q_wAAEgQ9LwEFA1YwACt8YxG2E9yUlC7K6N9S17bAEAAwIAA3kAAzUE"
        
        default_menu = [
            ("Оши палови тоҷикӣ", 40.0, "🍲 Таомҳои миллии гарм", OSH_IMAGE_ID),
...
def admin_router(msg):
    ...
    if txt == "👑 Саҳифаи Админ":
        ...
        conn.close()
        if not rows:
            bot.send_message(ADMIN_ID, "✅ Ҳамаи фармоишҳо иҷро шудаанд.", reply_markup=admin_kb())
            return
        mk = types.InlineKeyboardMarkup(row_width=1)
...
# ── Илова кардани таоми нав — қадамҳо ───────────────────────────
...
def client_router(msg):
    ...
    if msg.text == "👑 Саҳифаи Админ":
        if uid == ADMIN_ID:
            ...
            bot.send_message(uid, "❌ Танҳо Админ дастрасӣ doрад!", reply_markup=main_kb())
        return
    ...
    if msg.text == "📋 Менюи таомҳо":
        ...
        if not is_open():
            bot.send_message(
                ...
                "Фардо субҳ мунтазири шумо хоҳем буд 😊",
                parse_mode="Markdown",
                reply_markup=main_kb()
            )
            return
...
def cb_category(call):
    ...
    for name, price, img_id in items:
        ...
        mk.add(
            ...
            types.InlineKeyboardButton("➕ Илова кардан",    callback_data=f"add|{name}"),
        )
        caption = f"🍽 *{name}*\n💰 Нарх: {price:.2f} сомонӣ"
        if img_id:
            try:
                # Агар rасм бошад (ба монанди Ош), бо rасм мефиристад
...
def cb_cart(call):
    ...
    if action == "add":
        ...
        if row:
            ...
            conn.commit()
            bot.answer_callback_query(call.id, f"✅ {food_name} ба сабад илова шуd!")
        else:
            bot.answer_callback_query(call.id, "❌ Таом ёфт нашуд.")
    else:
        c.execute(
...
def handle_location(msg):
    ...
    # ── Тӯҳфа: Санҷиши локатсияи қабулкунанда ────────────────────
...
def start_booking(uid: int):
    ...
    if row and row[0]:
        ...
        mk = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
        mk.add(types.KeyboardButton("📱 Тасдиқи рақами... ", request_contact=True))
...
def step_guests(msg):
    ...
    if not msg.text or not msg.text.strip().isdigit() or int(msg.text.strip()) < 1:
        m = bot.send_message(uid, "❌ Хато! Танҳо рақами мусбатро ворид кунед:")
        bot.register_next_step_handler(m, step_guests)
        return
    guests = int(msg.text.strip())
    if guests > 50:
        m = bot.send_message(uid, "❌ Барои гурӯҳи калон бо администратор тамос гиред. Максимум 50 нафар:")
        bot.register_next_step_handler(m, step_guests)
        return
...
def step_day(msg):
    ...
    if not 1 <= day <= 31:
        m = bot.send_message(uid, "❌ Рӯз бояд байни 1 ва 31 бошад:")
        bot.register_next_step_handler(m, step_day)
        return
...
def step_month(msg):
    ...
    if not 1 <= month <= 12:
        m = bot.send_message(uid, "❌ Моҳ бояд байни 1 ва 12 бошад:")
        bot.register_next_step_handler(m, step_month)
        return
...
# ============================================================
#  📊  ҲИСОБОТИ РӮЗОНА — ҲАР ШАБ СОАТИ 21:00
# ============================================================
...
def handle_unknown(msg):
    ...
    bot.send_message(uid, "⚠️ Лутфан танҳо аз тугмаҳои мавҷуда истифода баред!", reply_markup=mk)

# ============================================================
#  🚀  ИШҒОЛ КАРДАН ВА САНҶИШИ ИНТЕРНЕТИ RENDER
# ============================================================
if __name__ == "__main__":
    print("✅ Боти ресторан оғоз шуд!")

    # Рӯйхати ҳисоботи рӯзона дар thread-и алоҳида
    t = threading.Thread(target=daily_report_loop, daemon=True)
    t.start()

    # САНҶИШИ СЕҲРНОК БАРОИ RENDER: 
    # Барои он ки Render "Timed out" накунад, мо як Порти оддии дохилиро ба кор меандозем.
    # Ин ба Render нишон медиҳад, ки бот зинда аст ва Портро гӯш карда истодааст.
    def run_dummy_server():
        import http.server
        import socketserver
        PORT = int(os.environ.get("PORT", 10000)) # Порти автоматии Render
        Handler = http.server.SimpleHTTPRequestHandler
        # Бо ин веб-сервери сохта, Render ҳеҷ гоҳ ботро хомӯш намекунад!
