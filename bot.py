...
def check_pending_gifts_for_user(uid: int, phone: str):
    ...
    if row:
        ...
        conn.close()
        name = msg.from_user.first_name if hasattr(msg, 'from_user') and msg.from_user else "дӯст"
        bot.send_message(uid, f"🏠 Менюи асосӣ кушода шуд. Хуш омадед!", reply_markup=main_kb())
...
# ============================================================
#  📍  ЛОКАТСИЯ (МАНТИҚИ НАВ БЕ ТОЗА КАРДАНИ САБАД)
# ============================================================
...
def handle_location(msg):
    ...
    # 1. Санҷиши умумии ҳудуди Душанбе
    if not in_dushanbe(lat, lon):
        mk = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
        mk.add(types.KeyboardButton("📍 Фиристодани локатсия", request_location=True))
        bot.send_message(
    ...
    # 2. Агар дар зали ресторан интихоб шуда бошад
...
def show_profile(msg):
    ...
    name  = msg.from_user.first_name or "Мизоҷ"
...
def handle_spam(msg):
    ...
    bot.send_message(
        msg.chat.id,
        "⚠️ Лутфан танҳо аз тугмаҳои мавҷуда исполда баред!",
        reply_markup=mk
    )
