...
import calendar
import threading
from datetime import datetime, timedelta, timezone  # <--- timezone илова шуд
import telebot
...
# Иловаи соатҳои кории нави шумо: 07:00 - 22:00
OPEN_HOUR      = 7
CLOSE_HOUR     = 22
...
def get_dushanbe_now() -> datetime:
    """Вақти ҷории Душанберо (UTC+5) бармегардонад, новобаста аз сервер"""
    dushanbe_tz = timezone(timedelta(hours=5))
    return datetime.now(dushanbe_tz)

def is_open() -> bool:
    # Санҷиши соат бо вақти Душанбе
...
def ensure_user(uid: int):
    """Ҳар як корбари навро ба базаи маълумот ворид ва эътироф мекунад"""
    conn = get_conn()
    c = conn.cursor()
    c.execute(
...
# ============================================================
#  🚀 /start (ҲАМАИ ОДАМОНРО ДАР БАЗА САБТ МЕКУНАД)
# ============================================================
...
def cmd_start(msg):
    ...
    # Илова ва захира кардани корбар дар база
    ensure_user(uid)
...
def admin_router(msg):
    ...
    if txt == "👑 Саҳифаи Админ":
        ...
        conn  = get_conn()
        c     = conn.cursor()
        today = get_dushanbe_now().strftime("%Y-%m-%d") # Бо вақти Душанбе
...
def cb_choose_status(call):
    ...
    try:
        ...
        bot.send_message(
            ADMIN_ID,
            f"📦 `{code}` — статуси naвро интихоб кунед:",
            parse_mode="Markdown",
            reply_markup=mk
        )
...
def process_gift_receiver(msg):
    ...
    if not receiver_id:
        bot.send_message(
            uid,
            "❌ Ин рақами телефон ҳанӯз дар боти мо рӯйхат نشدهаст (Корбар бояд ақаллан як бор ботро оғоз карда бошад)!\n"
...
def finalize_order(sender_id, receiver_id, o_type, details, code=None):
    ...
    # Сервер вақти эмас, Душанбе соати билан ёзилади
...
def step_month(msg):
    ...
    now  = get_dushanbe_now() # Санҷиши сана бо вақти Душанбе
    year = now.year
...
# ============================================================
#  📊 ҲИСОБОТИ РӮЗОНА — ҳар кеч соат 21:00 (ДУШАНБЕ ВАҚТИ БИЛАН)
# ============================================================
def daily_report_loop():
    while True:
        now = get_dushanbe_now() # Ички соат эмас, Душанбе вақти олинади
...
# ============================================================
#  🚫 ФИЛТРИ ПАЁМҲОИ БЕҲУДА (SPAM FILTER)
# ============================================================
