...
# ============================================================
#  ⚙️  ТАНЗИМОТИ АСОСӢ ВА ТАЪЙИНИ АВТОМАТИИ АДМИН
# ============================================================
TOKEN    = os.environ.get("BOT_TOKEN", "TOKEN_БА_ИН_ҶО")
# Танзимоти асосии соҳиби бот (Иброҳим)
MY_MAIN_ID = 1918349514  
ADMIN_ID   = int(os.environ.get("ADMIN_ID", MY_MAIN_ID))
...
def is_admin(uid: int) -> bool:
    # Санҷиш: агар корбар ID-и асосии ту ё ID-и танзимшударо дошта бошад, админ аст
    return uid == ADMIN_ID or uid == MY_MAIN_ID
...
# ============================================================
#  ⌨️  КЛАВИАТУРАҲО (ПРОФИЛИ МАН ПУРРА НЕСТ ШУД)
# ============================================================
def main_kb():
    m = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    m.add("📋 Менюи таомҳо", "🛒 Сабади харид")
    m.add("📅 Брон кардани стол")
    m.add("👑 Саҳифаи Админ")
    return m
...
def cmd_start(msg):
    ...
    # Сброси ҳолати админӣ агар корбар админ бошад
    if is_admin(uid):
...
def check_pending_gifts_for_user(uid: int, phone: str):
    ...
    if row:
        ...
        conn.close()
        bot.send_message(uid, "🏠 Менюи асосӣ кушода шуд. Хуш омадед!", reply_markup=main_kb())

# ============================================================
#  👑  ПАНЕЛИ АДМИН (ИҶОЗАТИ АВТОМАТӢ ИСЛОҲ ШУД)
# ============================================================
ADMIN_BTNS = {
    "👑 Саҳифаи Админ",
    "📊 Омори умумӣ",
...
def admin_router(msg):
    ...
    if txt == "👑 Саҳифаи Админ":
        ...
        bot.send_message(uid, txt2, parse_mode="Markdown", reply_markup=admin_kb())
        ...
        bot.send_message(uid, "Таоми дилхоҳро барои нест кардан интихоб кунед:", reply_markup=mk)
        ...
        conn.close()
        if not rows:
            bot.send_message(uid, "✅ Ҳамаи фармоишҳо иҷро шудааст.", reply_markup=admin_kb())
            return
        mk = types.InlineKeyboardMarkup(row_width=1)
        ...
        bot.send_message(uid, "Барои тағйир додани статус фармоишро интихоб кунед:", reply_markup=mk)
...
@bot.message_handler(func=lambda m: is_admin(m.chat.id) and m.chat.id in admin_state)
def admin_add_steps(msg):
    ...
    if step == "add_name":
        name = msg.text.strip()
        if len(name) < 2:
            bot.send_message(uid, "❌ Ном хеле кӯтоҳ аст. Дубора ворид кунед:")
            return
        state["name"] = name
        state["step"] = "add_price"
        bot.send_message(uid, "💰 Нархи таомро ворид кунед (масалан: 35.5):")
        ...
        try:
            ...
            for cat in CATEGORIES:
                mk.add(cat)
            bot.send_message(uid, "📁 Категорияро интихоб кунед:", reply_markup=mk)
        except ValueError:
            bot.send_message(uid, "❌ Нархи нодуруст! Масалан: 35.5")

    elif step == "add_category":
        if msg.text not in CATEGORIES:
            bot.send_message(uid, "❌ Лутфан аз рӯйхат интихоб кунед!")
            return
        conn = get_conn()
        c    = conn.cursor()
        try:
...
# ============================================================
#  📱  РОУТИНГИ КЛИЕНТ (ТУГМАИ ПРОФИЛ НЕСТ ШУД)
# ============================================================
CLIENT_BTNS = {
    "📋 Менюи таомҳо",
    "🛒 Сабади харид",
...
def process_gift_receiver(msg):
    ...
    if not row:
        ...
        bot.send_message(
            ...
            f"Азбаски ин шахс ҳанӯз дар бот нест, баробари ворид шудан ва пахш кардани /start, бот аз ӯ локатсияи воқеиро мепурсад.",
            reply_markup=main_kb()
        )
        return
...
# ============================================================
#  📍  ЛОКАТСИЯ (БЕ ТОЗА КАРДАНИ САБАД)
# ============================================================
...
def handle_location(msg):
    ...
    if uid in pending_orders:
        ...
        if not in_dushanbe(lat, lon):
            ...
            bot.send_message(
                uid,
                "⚠️ *Шумо берун аз ҳудуди шаҳри Душанбе ҳастед!*\n"
                "Лутфан, локатсияи воқеии худро дар дохили шаҳри Душанбе бифиристед:",
                reply_markup=mk
            )
            return
    ...
    stype = service_type.get(uid, None)
    if not stype:
        return
    ...
    if not in_dushanbe(lat, lon):
        ...
        bot.send_message(
            uid,
            "⚠️ *Локатсияи нодуруст!*\n\n"
            "Шумо берун аз ҳудуди шаҳри Душанбе қарор доред. Сабади шумо бетағйир монд.\n"
            "Лутфан, локатсияи дурусти худро дар дохили шаҳр бифиристед:",
            reply_markup=mk
        )
        return
    ...
    if stype == "🍽️ Дар зали ресторан":
        ...
        if dist > 150:
            ...
            bot.send_message(
                uid,
                f"⚠️ *Шумо дар зали ресторан нестед!*\n"
                f"Масофаи воқеии шумо: {dist:.0f} метр (иҷозат: то 150м).\n"
                f"Сабади харид тоза карда нашуд. Агар дар ресторан бошед, дубора кӯшиш кунед:",
                reply_markup=mk
            )
            return
        service_type.pop(uid, None)
...
def handle_spam(msg):
    try:
        ...
        pass
    mk = admin_kb() if is_admin(msg.chat.id) else main_kb()
    bot.send_message(
        msg.chat.id,
...
def handle_unknown(msg):
    ...
    mk = admin_kb() if is_admin(uid) else main_kb()
...
if __name__ == "__main__":
    print("✅ Боти ресторан бо муваффақият оғоз шуд!")
